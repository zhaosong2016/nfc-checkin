...
class NFCReader:
    ...
    def _init(self):
        try:
            ...
            self.reader=rl[0];print(f"[INFO] 已连接读卡器: {self.reader}");self.available=True
        except ImportError:print("[WARN] 未安装pyscard，演示模式")
        except Exception as e:print(f"[WARN] 读卡器初始化失败: {e}")
    def read_uid(self):
                ...
                if msg.get("type")=="manual_checkin":
                    ...
                    if 0<=pid<len(roster):
                        ...
                        if not p["checkedIn"]:
                            ...
                            await broadcast({"type":"checkin","id":p["id"],"name":p["name"],"time":p["time"],"bus":p.get("bus",1),"phone":p.get("phone","")})
